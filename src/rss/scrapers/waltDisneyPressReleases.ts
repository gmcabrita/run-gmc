...
export async function parse(response: Response): Promise<RSSData> {
  ...
  const rewriter = new HTMLRewriter()
    ...
    .on(
      ".component-article-filter .article-filter-results article.twdcus-card .twdcus-card__title a",
      {
        element(el) {
          if (!currentEntry) {
            return;
    ...
    .on(
      ".component-article-filter .article-filter-results article.twdcus-card time.twdcus-card__date",
      {
        element(el) {
          if (!currentEntry) {
            return;
