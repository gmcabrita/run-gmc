...
export function parse(json: DiscordQuestsResponse, nowDate: Date = new Date()): RSSData {
  ...
  const entries: RSSEntry[] = json.quests
    ...
    .map((quest) => {
      ...
      const enrolled = new Boolean(quest.user_status?.enrolled_at);
...
export async function get(ctx: ScraperContext): Promise<RSSData> {
  ...
  const rssData = parse(json);
  await Promise.all(
    rssData.entries.map(async (entry) => {
      if (!entry.extra?.enrolled) {
        await fetch(`https://discord.com/api/v9/quests/${entry.id}/enroll`, {
          headers: {
            accept: "*/*",
            "accept-language": "en-US,en;q=0.9,pt-PT;q=0.8,pt;q=0.7",
            authorization: ctx.env.DISCORD_AUTHORIZATION_TOKEN,
            priority: "u=1, i",
            referrer: "https://discord.com/quest-home?tab=all",
            "sec-ch-ua": '"Google Chrome";v="143", "Chromium";v="143", "Not A(Brand";v="24"',
            "sec-ch-ua-mobile": "?0",
            "sec-ch-ua-platform": '"macOS"',
            "sec-fetch-dest": "empty",
            ...
            "x-discord-timezone": "Europe/Lisbon",
            "x-super-properties":
              "",
          },
          method: "POST",
          body: '{"location":11,"is_targeted":false,"metadata_raw":null,"metadata_sealed":null}',
        });
      }
    }),
  );
