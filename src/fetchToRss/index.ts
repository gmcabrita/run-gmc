...
function* generateNextDatesCinemateca() {
...
export function addFetchToRssEndpoints(app: Hono<{ Bindings: CloudflareBindings }>) {
  ...
  app.get("/rss.cinemateca", async (c) => {
    ...
    const scrapeUrls = Array.from(generateNextDatesCinemateca()).map(
  ...
  app.get("/rss.waltDisneyPressReleases2", async (c) => {
    ...
    const rewriter = new HTMLRewriter()
      ...
      .on(".press-releases-container > article h2 a", {
        element(el) {
          ...
          if (lastEntry) {
            lastEntry.link = el.getAttribute("href") || "";
          }
        },
        text(text) {
          const lastEntry = entries[entries.length - 1];
      ...
      .on(".press-releases-container > article time", {
        element(el) {
          const lastEntry = entries[entries.length - 1];
          if (lastEntry) {
            lastEntry.datetime = el.getAttribute("datetime");
          }
        },
      });
    ...
    entries.forEach((entry) => {
      feed.addItem({
        ...
        link: entry.link,
        content:
          `<p>${entry.content}</p><a href="${entry.link}">${entry.link}</a>${entry.image || ""}`.trim(),
        date: entry.datetime ? new Date(entry.datetime) : now,
      });
    });
