...
async function fetchUserId(env: CloudflareBindings, userName: any): Promise<string> {
...
async function fetchPosts(env: CloudflareBindings, userId: any): Promise<any> {
  const response = await fetch(
...
async function transformPost(env: CloudflareBindings, post: any): Promise<any> {
...
async function getEmbed(env: CloudflareBindings, postUrl: string): Promise<any> {
...
async function x2Rss(env: CloudflareBindings, userName: string, data: any): Promise<any> {
  const firstUser = data.data.user.result.timeline_v2.timeline.instructions.find(
    (instruction: any) => instruction.entries,
